...
async function seedSales() {
    try {
        ...
        const users = await User_1.default.find({ role: { $in: ['sh', 'hba', 'hcm', 'hcc'] } });
...
async function addLedger(userId, amount, type, desc, date) {
    const wallet = await Wallet_1.default.findOne({ user: userId });
    if (wallet) {
