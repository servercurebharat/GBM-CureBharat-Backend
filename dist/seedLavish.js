...
async function seed() {
    try {
        ...
        // Find Amit Mishra
        const amit = await User_1.default.findOne({ mobile: '9826490451' });
        if (!amit) {
            console.log('Amit Mishra not found!');
            process.exit(1);
        }
        // 1. Create Lavish Kulkarni
        const mobile = '9584258103';
        ...
        // 2. Buy the Plan (Create Sale by Amit Mishra to Lavish Kulkarni)
        const plan = await Plan_1.default.findOne({ price: 199900 });
        if (!plan) {
        ...
        console.log('✅ Done seeding Lavish Kulkarni under Amit Mishra!');
        process.exit(0);
    }
    catch (error) {
