...
async function seedRajeshHierarchy() {
    ...
    const rajesh = await User_1.default.findOne({ memberId: 'CB-SH-0001' });
    if (!rajesh) {
        console.error('❌ Rajesh Patel (CB-SH-0001) not found!');
        process.exit(1);
    }
    console.log(`✅ Found Rajesh Patel: ${rajesh._id}`);
    const hashedPwd = await bcryptjs_1.default.hash('password123', 10);
    // ============================================================
    ...
    for (const d of hbaData) {
        const existing = await User_1.default.findOne({ memberId: d.memberId });
        if (existing) {
    ...
    for (const d of hcmData) {
        ...
        const hcm = await User_1.default.create({
            name: d.name,
            mobile: d.mobile,
            password: hashedPwd,
            role: 'hcm',
