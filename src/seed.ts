...
async function seed() {
  try {
    ...
    const admin = await User.create({
      ...
      password: await bcrypt.hash('Admin@123', 12), role: 'admin', rank: 'ADMIN',
    ...
    const sh = await User.create({
      ...
      password: await bcrypt.hash('SH@123456', 12), role: 'sh', rank: 'SH',
    ...
    // ============================================================
    const hba1 = await User.create({
      name: 'Sanjay Mehta', mobile: '9200000001', email: 'sanjay.hba@curebharat.in',
      password: await bcrypt.hash('HBA@123456', 12), role: 'hba', rank: 'HBA',
    ...
    await Wallet.create({ user: hba1._id });

    // HBA 2: Meena Joshi
    const hba2 = await User.create({
    ...
    await Wallet.create({ user: hba2._id });

    // HBA 3: Arvind Kumar
    const hba3 = await User.create({
    ...
    await Wallet.create({ user: hba3._id });
    console.log('✅ 3 HBAs created under Rajesh Patel');

    // ============================================================
    // HCMs under HBA 1 (Sanjay Mehta)
    // ============================================================
    const hcmPwd = await bcrypt.hash('HCM@123456', 12);
    ...
    await Wallet.create({ user: hcm3._id });

    const hcm4 = await User.create({
    ...
    await Wallet.create({ user: hcm4._id });

    // 1 HCM under HBA 3 (Arvind Kumar)
    const hcm5 = await User.create({
    ...
    await Wallet.create({ user: hcm5._id });
    console.log('✅ 5 HCMs created');

    // ============================================================
    // HCCs under HCM 1 (Priya Desai)
    // ============================================================
    const hccPwd = await bcrypt.hash('HCC@123456', 12);
