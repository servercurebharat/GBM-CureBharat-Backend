Anandkumar kalalimport mongoose from 'mongoose';
...
async function seed() {
  try {
    ...
    // Find HJ Finance
    const hjFinance = await User.findOne({ name: /HJ Finance/i });
    if (!hjFinance) {
      console.log('HJ Finance not found!');
      process.exit(1);
    }

    // 1. Create Jaysukhbhai
    const mobile = '7405077077';
    ...
    if (!customer) {
      ...
      customer = new User({
        name: 'Jaysukhbhai Sonchhatra',
        mobile,
        email: 'jaysukhsonchhatra@gmail.com',
        state: 'Gujarat',
        password: '123456',
        role: 'hcc',
        rank: 'HCC',
    ...
    // 2. Buy the Plan (Create Sale by HJ Finance to Jaysukhbhai)
    const plan = await Plan.findOne({ name: { $regex: /Super Suraksha/i } });
    if (!plan) {
    ...
    if (existingSale) {
      ...
      const sale = new Sale({
        ...
        plan: plan._id,
        customerName: 'Jaysukhbhai Sonchhatra',
    ...
    console.log('✅ Done seeding Jaysukhbhai under HJ Finance!');
    process.exit(0);
  } catch (error) {
