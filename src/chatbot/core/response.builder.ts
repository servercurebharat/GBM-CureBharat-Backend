import { ChatbotIntent, ChatbotResponse } from '../types/chatbot.types';
import { getFAQByRole } from '../faq';
import { getWalletBalance, getWalletHistory } from '../database/wallet.query';
import { getKYCStatus, getTeamSize, getSponsor, getUserRank, getTeamMembers } from '../database/users.query';
import { getSalesCount, getLatestSale } from '../database/sales.query';
import { getEPinCount } from '../database/epins.query';
import { handleFallback } from '../utils/fallback.engine';
...
export const buildResponse = async (
  ...
  for (const faq of faqs) {
    ...
    // PARTIAL QUESTION MATCH
      ...
      faqQuestion.includes(lowerMessage) ||
  ...
  const dynamicIntents: ChatbotIntent[] = [
    'wallet_balance',
    'wallet_history',
    'wallet_tds',
    'sales_count',
  ...
  const isDynamicIntent =
    dynamicIntents.includes(intent) &&
    confidenceScore >= 0.6;
  ...
  let type: 'static' | 'dynamic' | 'hybrid' = 'static';
  let answer = "I'm not sure how to answer that yet.";
  let data = null;
  let suggestions: string[] = ['What is my wallet balance?', 'What is my KYC status?'];
  ...
  try {
    switch (intent) {
      ...
      case 'team_growth': {
        ...
        if (teamMembers.length === 0) {
          ...
          Object.keys(grouped).forEach((role) => {
            ...
            grouped[role].forEach((member: any, index: number) => {
      ...
      case 'sales_count': {
        type = 'dynamic';
        const count = await getSalesCount(userId, role);
      ...
      case 'sales_latest': {
        type = 'dynamic';
        const sale = await getLatestSale(userId, role);
        if (sale) {
      ...
      case 'epin_status': {
        type = 'dynamic';
        const counts = await getEPinCount(userId);
      ...
      case 'faq_help': {
        ...
        const matchedFaq = faqs.find(f =>
          f.keywords.some(kw => message.toLowerCase().includes(kw.toLowerCase()))
        );
